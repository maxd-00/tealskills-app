  ...
  async function loadMonths(yid) {
    try {
      const { data, error } = await supabase
        ...
        .eq("year_id", yid);
      if (error) throw error;
      const sorted = (data || []).slice().sort((a, b) => idx[a.month] - idx[b.month]);
      setMonths(sorted);
    } catch (e) {
      console.error(e);
...
function AdminRoles_Assign() {
  ...
  // Brouillon local : userId -> selectedRoleId (UUID string) ou ""
  ...
  function isDirty(user) {
    const current = currentRoleIdFromDB(user);
        ...
        <table className="min-w-[880px] w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="text-left p-3">Role</th>
              <th className="text-left p-3 w-[200px]">Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
